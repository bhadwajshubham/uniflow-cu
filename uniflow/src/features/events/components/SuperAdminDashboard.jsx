...
import { 
  collection, getDocs, doc, updateDoc, query, limit, onSnapshot, setDoc 
} from 'firebase/firestore';
import { db } from '../../../lib/firebase';
import { 
  Shield, Users, Mail, Search, Zap, BarChart3, RefreshCcw, UserMinus 
} from 'lucide-react';
...
const SuperAdminDashboard = () => {
  ...
  // 🔥 FIXED: REVOKE ACCESS FUNCTION
  const handleRevokeAccess = async (userId, currentRole) => {
    if (currentRole === 'super_admin') {
      alert("Cannot revoke access from a Super Admin.");
      return;
    }
    if (!window.confirm("Are you sure you want to revoke Admin access? This user will become a student.")) return;
    
    try {
      await updateDoc(doc(db, 'users', userId), {
        role: 'student' // Changes role back to default
      });
  ...
  if (loading) return <div className="min-h-screen pt-32 text-center font-black text-red-600">ACCESSING SYSTEM CORE...</div>;

  return (
    <div className="min-h-screen bg-[#FDFBF7] dark:bg-black pt-24 pb-24 px-4 sm:px-6">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-4xl font-black mb-8 tracking-tighter dark:text-white italic uppercase">SuperAdmin Panel</h1>
        
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
           <div className={`p-8 rounded-[2.5rem] border ${systemStats.isEmailActive ? 'bg-white border-zinc-200' : 'bg-red-50 border-red-200'} dark:bg-zinc-900`}>
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={toggleEmailSystem} className="text-[10px] font-black uppercase text-red-600 underline underline-offset-4">Toggle Power</button>
              </div>
              <div className="text-3xl font-black dark:text-white">{systemStats.count} / 500</div>
              <p className="text-xs text-zinc-500 font-bold uppercase mt-1 tracking-widest">Daily Email Traffic</p>
           </div>
           ...
           <div className="p-8 bg-white dark:bg-zinc-900 rounded-[2.5rem] border border-zinc-200 dark:border-zinc-800 shadow-sm">
              ...
              <p className="text-xs text-zinc-500 font-bold uppercase tracking-widest mt-1">Platform Users</p>
           </div>
           ...
           <div className="p-8 bg-zinc-900 dark:bg-white text-white dark:text-black rounded-[2.5rem] shadow-xl">
              ...
              <div className="text-3xl font-black tracking-tighter uppercase font-serif italic">Optimal</div>
              <p className="text-xs opacity-50 font-bold uppercase mt-1 tracking-widest">System Health</p>
           </div>
        </div>
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-[2.5rem] border border-zinc-200 dark:border-zinc-800 overflow-hidden shadow-xl">
           <div className="p-6 border-b border-zinc-100 dark:border-zinc-800 flex justify-between items-center bg-zinc-50/50 dark:bg-black/20">
              <h2 className="font-black text-xl tracking-tighter dark:text-white">Student Governance</h2>
              <div className="relative">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-400" />
                <input type="text" placeholder="Search by name/email..." className="pl-10 pr-4 py-2 bg-white dark:bg-black border border-zinc-200 rounded-xl text-xs outline-none focus:ring-2 focus:ring-indigo-600/20 dark:text-white" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
              </div>
           </div>
                     ...
                     <tr key={user.id} className="hover:bg-zinc-50/50 dark:hover:bg-indigo-900/5 transition-colors group">
                        ...
                        <td className="px-8 py-5">
                           <span className={`px-2 py-1 rounded-lg text-[10px] font-black uppercase tracking-tight ${
                             user.role === 'admin' || user.role === 'super_admin' ? 'bg-indigo-100 text-indigo-600' : 'bg-zinc-100 text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400'
                           }`}>
                              {user.role || 'student'}
                           </span>
                        </td>
                        <td className="px-8 py-5 text-right">
                           {user.role !== 'super_admin' && user.role === 'admin' && (
                             <button 
                               onClick={() => handleRevokeAccess(user.id, user.role)}
                               className="flex items-center gap-1 text-xs font-black text-red-600 hover:underline ml-auto"
                             >
