...
const RegisterModal = ({ event, onClose, isOpen }) => {
  ...
  // Handle both old and new event type logic
        ...
        <div className="p-8 overflow-y-auto custom-scrollbar">
          {step === 'choice' ? (
            <div className="space-y-4">
              <p className="text-xs font-black text-zinc-400 uppercase tracking-wider mb-2">Select Participation</p>
              ...
              {!isTeamEvent && (
                <button onClick={() => handleModeSelect('solo')} className="w-full p-6 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-[1.5rem] hover:border-indigo-500 transition-all group flex items-center gap-4 text-left shadow-sm">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-[10px] text-zinc-500 font-bold uppercase">Solo Entry</p>
                  </div>
                ...
                <>
                  <button onClick={() => handleModeSelect('create_team')} className="w-full p-6 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-[1.5rem] hover:border-purple-500 transition-all group flex items-center gap-4 text-left shadow-sm">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-[10px] text-zinc-500 font-bold uppercase">Create & Invite</p>
                    </div>
                    <ArrowRight className="h-4 w-4 text-zinc-300" />
                  </button>

                  <button onClick={() => handleModeSelect('join_team')} className="w-full p-6 border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-[1.5rem] hover:border-zinc-400 transition-all flex items-center gap-4 text-left">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-[10px] text-zinc-500 font-bold uppercase">Enter Team Code</p>
                    </div>
                  </button>
                </>
              )}
            ...
            <form onSubmit={handleRegister} className="space-y-6">
              ...
              <div className="p-4 bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-900/30 rounded-2xl flex gap-3">
                 <AlertTriangle className="w-5 h-5 text-red-600 shrink-0" />
                 <div>
                    <p className="text-[10px] font-black text-red-600 uppercase tracking-widest">Eligibility Alert</p>
                    <p className="text-[10px] font-bold text-zinc-700 dark:text-zinc-300 mt-1 uppercase leading-tight">
                      This event is restricted to <span className="underline italic">C.S.E. & C.S.E. (AI)</span> only. @chitkara ID is mandatory.
                    </p>
                 </div>
              </div>
              ...
              <div className="flex items-center justify-between">
                <p className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">Student Information</p>
                {isTeamEvent && (
              ...
              <div className="space-y-4">
                <div className="relative">
                  ...
                  <input required type="text" placeholder="University Roll Number" className="w-full pl-12 pr-4 py-4 bg-zinc-100 dark:bg-black border-none rounded-2xl text-sm font-bold dark:text-white outline-none focus:ring-2 focus:ring-indigo-500/20"
                    value={studentData.rollNo} onChange={e => setStudentData({...studentData, rollNo: e.target.value})} />
                </div>
                
                {/* 🎯 BRANCH DROPDOWN (Strict Lockdown) */}
                <div className="relative">
                  <GraduationCap className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-zinc-400" />
                  <select required className="w-full pl-12 pr-4 py-4 bg-zinc-100 dark:bg-black border-none rounded-2xl text-sm font-bold dark:text-white appearance-none outline-none"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="relative">
                    ...
                    <input required type="tel" placeholder="Phone" className="w-full pl-10 pr-4 py-4 bg-zinc-100 dark:bg-black border-none rounded-2xl text-sm font-bold dark:text-white outline-none"
                ...
                <select required className="w-full px-4 py-4 bg-zinc-100 dark:bg-black border-none rounded-2xl text-sm font-bold dark:text-white appearance-none outline-none"
              ...
              {mode === 'create_team' && (
                <div className="pt-4 border-t border-zinc-100 dark:border-zinc-800">
                  ...
                  <input required type="text" placeholder="The Code Warriors" className="w-full px-4 py-4 bg-indigo-50 dark:bg-indigo-900/10 border-none rounded-2xl text-sm font-bold dark:text-white outline-none"
              ...
              <div className="space-y-3">
                ...
                <label className="flex items-center justify-between p-4 bg-white dark:bg-zinc-800 border border-zinc-100 dark:border-zinc-700 rounded-2xl cursor-pointer">
                  <div className="flex flex-col">
                    <span className="text-xs font-black dark:text-white tracking-tight text-sm">Social Discovery</span>
                    <span className="text-[10px] text-zinc-400 font-bold uppercase">Show in "Who's Going"</span>
                  </div>
              ...
              <button type="submit" disabled={loading || !isStudentInfoComplete}
                className="w-full py-4 bg-zinc-900 dark:bg-white text-white dark:text-black rounded-2xl font-black text-xs uppercase tracking-[0.3em] shadow-xl shadow-indigo-500/10 disabled:opacity-20 active:scale-95 transition-all">
